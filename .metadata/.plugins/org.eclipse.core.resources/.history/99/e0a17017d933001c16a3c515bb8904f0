...
public class TestPerformer implements CJProxy {
	...
	TestPerformer(int blackBoxesNumber, int testMode, int desiredBitArraySize, int encryptionOrder) {
		if (encryptionOrder < 0) {
			System.err.println("Liczba iteracji algorytmu powinna byæ liczb¹ nieujemn¹.");
			resetTest();
			return;
		}
	...
	private boolean validateInput(int blackBoxesNumber, int testMode, int desiredBitArraySize) {
		if (blackBoxesNumber < 1) {
			System.err.println("Liczba powtórzeñ testu powinna byæ wiêksza od 0.");
			return false;
		}
		if (testMode < 1 || testMode > 11) {
			System.err.println("Numer testu powinien byæ liczb¹ naturaln¹ z zakresu 1 - 11.");
			return false;
		}
		if (desiredBitArraySize < 1) {
			System.err.println("D³ugoœæ ci¹gu bitów powinna byæ wiêksza od 0.");
			return false;
		}
		return true;
	}
	...
	public Object execute(Object args) throws Exception {
		if (!(args instanceof int[])) {
			throw new Exception("Argument metody 'execute' powinien byæ tablic¹ prymitywów typu int.");
		}
		int[] arguments = (int[]) args;
		if (arguments.length < 1) {
			throw new Exception("Zbyt ma³o argumentów przekazanych w tablicy bêd¹cej argumentem " + "metody 'execute'.");
		}

		int blackBoxNumber = arguments[0];
		if (_blackBoxesGenerator == null) {
			throw new Exception("Wyst¹pi³ b³¹d podczas inicjalizacji testu.");
		}
		if (blackBoxNumber >= _blackBoxesGenerator.getSize()) {
			throw new Exception("Numer powtórzenia testu jest wiêkszy ni¿ ca³kowita liczba powtórzeñ.");
		}
	...
	private void printDebugLog(int blackBoxNumber, int i, int j, int loopsCount, long startTime) {
		...
		System.out.println("BB: " + blackBoxNumber + " loopsCount: " + loopsCount + " i: " + i + " j: " + j + ". up³ynê³o: "
	...
	// + " i: " +
	// i + " j: " + j + ". up³ynê³o: " + (System.currentTimeMillis() -
	// startTime) /
