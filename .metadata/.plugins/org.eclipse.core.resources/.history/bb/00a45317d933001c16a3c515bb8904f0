...
public class BlackBox {
	...
	private Alphabet checkAlphabet(Alphabet alphabet) {
		if (alphabet == null || alphabet.getSize() < 2) {
			System.err.println("Alphabet jest za ma³y by za³o¿enia co do miksów mog³y byæ " + "spe³nione. U¿ywam domyœlnego alfabetu.");
			return new Alphabet();
		} else {
			return alphabet;
		}
	...
	public void validateAndSetCommonEntries(List<Boolean> k, List<Boolean> r, int n) {
		if (k == null || k.isEmpty()) {
			System.err.println("Ustawiam domyœlne s³owo 'k'.");
		} else {
			_k = k.toArray(new Boolean[0]);
			_kSize = _k.length;
		}
		if (r == null || r.isEmpty()) {
			System.err.println("Ustawiam domyœlne s³owo 'r'.");
		} else {
	...
	public void validateAndSetOrder(int n) {
		if (n < 0) {
			System.err.println("Ustawiam domyœln¹ liczbê iteracji algorytmu 'n'.");
		} else {
			_n = n;
			_2n = n << 1;
		}
		...
		synchronized (encryptionLock) {
			...
			for (int i = 0; i < _encryptionCharsLength; i++) {
				...
				if (!_alphabet.hasLetter(c)) {
					throw new Exception("S³owo zawiera literê: '" + c + "', która nie wystêpuje w alfabecie.");
				}
				Character oldv = v;
	...
	public String encrypt(String word) {
		if (word == null || word.isEmpty()) {
			System.err.println("Poda³eœ puste s³owo do zaszyfrowania.");
			return null;
		}
		...
		if (encryptedWord == null) {
			System.err.println("Wyst¹pi³ b³¹d podczas szyfrowania wiadomoœci.");
			return null;
		}
	...
	private void dec() throws Exception {
		synchronized (decryptionLock) {
			...
			for (int i = 0; i < _decryptionCharsLength; i++) {
				...
				if (!_alphabet.hasLetter(c)) {
					throw new Exception("Kryptotekst zawiera literê: '" + c + "', która nie " + "wystêpuje w alfabecie.");
				}
	...
	public String decrypt(String cryptotext) {
		if (cryptotext == null || cryptotext.isEmpty()) {
			System.err.println("Poda³eœ puste s³owo do odszyfrowania.");
			return null;
		}
		Character firstLetter = _alphabet.getLetter(0);
		...
		if (decryptedWord == null) {
			System.err.println("Wyst¹pi³ b³¹d podczas deszyfrowania wiadomoœci.");
			return null;
		}
		...
		if (changeRKey) {
			for (int changePosition : changePositions) {
				if (changePosition < _r.length) {
					...
					System.err.println("Klucz r (d³ugoœci " + _r.length + ") jest zbyt krótki, by zmieniæ jego pozycjê nr " + changePosition + ".");
				}
			}
		} else {
			for (int changePosition : changePositions) {
				if (changePosition < _r.length) {
					...
					System.err.println("Klucz k (d³ugoœci " + _k.length + ") jest zbyt krótki, by zmieniæ jego pozycjê nr " + changePosition + ".");
				}
			}
		}
	}
	...
	public BlackBox[] getBlackBoxesWithChangedMixesQYForTests(int letterNumber, int[] swaps) {
		...
		for (int i = letterNumber + 1; i < _alphabet.getSize(); i++) {
			...
			// System.out.println("--------------------- wyjœciowy q: ");
