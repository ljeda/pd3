...
public class PerformanceTester {
	...
	private void validateInput(int blackBoxesNumber, int n, int keysLength, int sampleSize) throws Exception {
		if (blackBoxesNumber < 1) {
			throw new Exception("Liczba powtórzeñ testu powinna byæ wiêksza od 0.");
		}
		if (n < 0) {
			throw new Exception("Liczba iteracji algorytmu powinna byæ nieujemna.");
		}
		if (keysLength < 1) {
			throw new Exception("D³ugoœæ kluczy powinna byæ wiêksza od 0.");
		}
		if (sampleSize < 1) {
			throw new Exception("D³ugoœæ próbki powinna byæ wiêksza od 0.");
		}
	}

	public void performTest(String subfolder) {
		if (_blackBoxesGenerator == null) {
			System.err.println("Instancje do testów zosta³y zainicjalizowane niepoprawnie.");
			return;
		}
		if (_sample == null) {
			System.err.println("Próbka do testów zosta³a zainicjalizowana niepoprawnie.");
			return;
		}
		for (int i = 0; i < _blackBoxesGenerator.getSize(); i++) {
			try {
				...
				if (!_sample.equals(decryptedSample)) {
					System.err.println("Algorytm zosta³ zaburzony, " + "zdekodowana wiadomoœæ jest inna ni¿ wyjœciowa.");
					break;
				}
				...
				System.err.println("Wyst¹pi³ b³¹d przy próbie pobrania instancji " + "do testów wydajnoœciowych: " + e.getLocalizedMessage());
				continue;
			}
		}
	...
	private void writeResults(String subfolder) {
		...
		String rootFolder = "D:\\studia\\III stopieñ\\doktorat\\workspace\\PD3\\performance_tests\\";
		...
		try {
			...
			System.err.println("Wyst¹pi³ b³¹d przy próbie stworzenia folderu do zapisu wyników testów wydajnoœciowych: " + e.getLocalizedMessage());
		}
		...
		try {
			...
			System.err.println("Zapisywanie wyników do pliku nie powiod³o siê: " + e.getLocalizedMessage());
		} finally {
			if (writer != null) {
				writer.close();
			}
